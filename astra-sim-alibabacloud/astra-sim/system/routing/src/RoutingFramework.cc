...
void RoutingFramework::PrecalculateRoutingTables() {
    ...
    for (int src_id = 0; src_id < node_count; src_id++) {
        int ntype = topology_.GetNodeType(src_id);
        // Build routing table for any forwarding element: Host (0), Switch (1), NVSwitch (2)
...
void RoutingFramework::CalculateRouteNS3Style(int host_node) {
    ...
    // Pure hop-count ECMP: do not pre-prefer NVSwitch; treat all nodes equally
