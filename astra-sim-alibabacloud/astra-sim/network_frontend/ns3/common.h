...
// Global routing framework instance
AstraSim::RoutingFramework* g_system_routing = nullptr;
...
void SetupNetwork(void (*qp_finish)(FILE *, Ptr<RdmaQueuePair>),void (*send_finish)(FILE *, Ptr<RdmaQueuePair>), const std::string& result_dir = "results/ns3/") {
  ...
  // Initialize routing framework from system
  ...
  for (uint32_t i = 0; i < node_num; i++) {
    if (n.Get(i)->GetNodeType() == 1) { 
      Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n.Get(i));
      uint32_t shift = 1;  // More generous PFC threshold (was 1) 
      ...
      for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
        ...
        uint64_t delay = DynamicCast<QbbChannel>(dev->GetChannel())
                             ->GetDelay()
                             .GetTimeStep();
        uint32_t headroom = rate * delay / 8 / 1000000000 * 20;
